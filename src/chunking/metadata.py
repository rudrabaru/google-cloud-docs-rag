...
class ChunkMetadata(BaseModel):
    ...
    # Structural metadata
    heading_path: List[str] = Field(default_factory=list, description="Hierarchy of headings leading to this chunk")
    ...
    code_languages: Optional[List[str]] = Field(None, description="Languages of code blocks")
    contains_table: bool = Field(False, description="Chunk contains markdown tables")
    content_type: str = Field("mixed", description="Primary content type: text, code, table, mixed")
    ...
    oversized_chunk: bool = Field(False, description="Is this chunk > max target size but allowed as exception")
    tiny_chunk_merged: bool = Field(False, description="Was this chunk created by merging tiny chunks")
