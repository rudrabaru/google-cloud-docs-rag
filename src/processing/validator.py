...
from src.processing.models import ProcessedDocument, ProcessingStats

class ProcessingValidator:
    
    @staticmethod
    def validate_document(raw_content: str, processed_blocks: list, doc: ProcessedDocument):
        ...
        stats.code_blocks_before = len(re.findall(r'^```', raw_content, re.MULTILINE)) // 2
        ...
        stats.code_blocks_after = len(re.findall(r'^```', after_content, re.MULTILINE)) // 2
    ...
    def generate_corpus_report(documents: List[ProcessedDocument]) -> Dict:
        ...
        return {
            ...
            "overall_retention": (words_after / words_before * 100) if words_before else 0,
            "code_preservation": {
                "before": code_before,
                "after": code_after,
            ...
            "block_statistics": {
                ...
                "removal_rate": (blocks_removed / (blocks_removed + blocks_retained) * 100) if (blocks_removed + blocks_retained) > 0 else 0
            }
        }
