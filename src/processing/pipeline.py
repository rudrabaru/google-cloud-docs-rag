...
class ProcessingPipeline:
    ...
    def run(self):
        ...
        sorted_blocks = sorted(
            cleaner.block_document_counts.items(), key=lambda x: x[1], reverse=True
        )
        top_boilerplate = [
            {"hash": k, "count": v, "percentage": (v / total_docs) * 100}
            for k, v in sorted_blocks[:100]
        ]
        with open(
            self.output_dir / "metrics" / "repeated_blocks_report.json",
            "w",
            encoding="utf-8",
        ) as f:
        ...
        with open(
            self.output_dir / "metrics" / "removed_blocks_report.json",
            "w",
            encoding="utf-8",
        ) as f:
        ...
        with open(
            self.output_dir / "metrics" / "boilerplate_detection_report.json",
            "w",
            encoding="utf-8",
        ) as f:
            json.dump(
                {
                    "total_blocks_processed": sum(
                        len(d.blocks) for d in processed_docs
                    ),
                    "signals_triggered": signal_counts,
                },
                f,
                indent=2,
            )
    ...
    def _generate_review_set(self, processed_docs: List[ProcessedDocument]):
        ...
        for doc in review_samples:
            removed_sections = [
                {
                    "content": b.content,
                    "reason": b.removal_reason,
                    "score": b.boilerplate_score,
                    "signals": getattr(b, "triggered_signals", []),
                }
                for b in doc.blocks
                if b.is_removed
            ]
            preserved_sections = [
                ...
                for b in doc.blocks
                if not b.is_removed and b.boilerplate_score < 4.0
            ]
            borderline_sections = [
                {
                    "content": b.content,
        ...
        with open(
            self.output_dir / "review" / "review_set.json", "w", encoding="utf-8"
        ) as f:
